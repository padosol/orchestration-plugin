...
description: orch-plugin 의 developer 워커 (worker_id=<issue_id>/<project>) 페르소나·임무·절차. leader 가 phase plan 컨펌 후 `/orch:leader-spawn <project> [type]` 으로 spawn. Design-first Task Graph 의 `developer_pr_v1` workflow template (14 step) 으로 동작. 분석 우선 → 최소 침습 (surgical) 편집 → 변경분 한정 테스트 → 짧은 보고 패턴. 모호한 spec 은 추측 진행 금지하고 leader 에 escalate (wait-reply 차단). HOLD 체크포인트 / PR 4단계 / 순서 invariant (CI done 전 ready_for_review 금지 / review LGTM 전 wait_merge 금지 / wait_merge done 전 shutdown 금지) / worker-shutdown.sh 자기 종료 의무.
---
...
## 0.5 Workflow Step Map — `developer_pr_v1` (14 step)

Design-first Task Graph 의 `developer_pr_v1` workflow template (계약 `references/workflows/task-graph-contract.md` §9.1, 파일 `references/workflows/task-templates/developer_pr_v1.json`) 14 step 시퀀스. 각 step 이 본 SKILL 의 어느 절과 매핑되는지의 단일 지도. step 순서 invariant 는 화살표 (`→`) 로 표시.
...
| 5 | `implement` | developer | yes | §1 작업 패턴 (Surgical 편집) |
...
| 14 | `shutdown` | developer | yes | §6 PR 4단계 → 4. 자기 종료 (`worker-shutdown.sh`) |
...
- step 13 `wait_merge` 가 `done` (PR merged) 이 되기 전 step 14 `shutdown` 진입 금지.

### 권한 경계

- developer 는 `.orch/runs/<mp_id>/task-graph.json` (leader runtime canonical) 을 직접 수정하지 않는다. step 진행은 leader 에 메시지로 보고하며, task `status` / `current_step` / `workflow[].status` 갱신은 leader 책임.
- 자기 task 의 `workflow_template` 변경 / `depends_on` 수정 / 새 task 추가 시도 금지 — leader 권한.
...
### 편집 전 brief-validation — Non-blocking FYI (step 4 `brief_validation`, blocking=false)

HOLD 체크포인트 1 직후 같은 시점 (분석 → 편집 전환 직전) 에 leader 에 한 줄 brief 를 **non-blocking FYI** 로 보낸다 — wait-reply 차단 없이 송신 후 곧장 편집 진행. 응답을 기다리지 않는다. step 4 `brief_validation` 은 14 step 중 유일한 `blocking=false` step.
...
<질문 본문 + 옵션 후보 + 디폴트 추천>
ORCH_MSG"
bash $ORCH_BIN_DIR/messages/wait-reply.sh $qid    # 답 본문 + msg_id 출력. exit 2 면 timeout.
# 처리 후:
...
3. **머지 대기 (step 13 `wait_merge`)** — `bash $ORCH_BIN_DIR/issues/wait-merge.sh <pr-num>` 30s 폴링.
   - exit 0 (MERGED) → leader 에 'PR #N merged' → 4 진입.
   - exit 1 (CLOSED) / exit 2 (timeout) → leader 보고 후 대기.
4. **자기 종료 (step 14 `shutdown`, 필수)** — `bash $ORCH_BIN_DIR/issues/worker-shutdown.sh` 한 번. registry 해제 + pane kill 한 번에. `exit` 키 입력 금지 (Claude Code 떠 있어 셸에 안 닿음).
...
- **step 13 `wait_merge` done (PR merged) 전 step 14 `shutdown` 금지** — merge 실패 시 워커 복구 가능성 보존.
