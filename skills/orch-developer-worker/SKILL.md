...
3. **머지 대기 (step 13 `wait_merge`)** — `bash $ORCH_BIN_DIR/wait-merge.sh <pr-num>` 30s 폴링.
   - exit 0 (MERGED) → leader 에 'PR #N merged' → 4 진입.
   - exit 1 (CLOSED) / exit 2 (timeout) → leader 보고 후 대기.
4. **자기 종료 (step 14 `shutdown`, 필수)** — `bash $ORCH_BIN_DIR/worker-shutdown.sh` 한 번. registry 해제 + pane kill 한 번에. `exit` 키 입력 금지 (Claude Code 떠 있어 셸에 안 닿음).
