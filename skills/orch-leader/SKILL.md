...
1. **이슈 컨텍스트 fetch** — `<issue_fetch_step>` 의 primary 명령 실행 (linear / github / gitlab / jira / none 분기는 first_msg 가 결정해 줌). spec / acceptance criteria / labels / title 확보.

   ### 1.1 fetch 실패 fallback (fuzzy 후보 → 사용자 질문)

   트래커가 "이슈 없음 / 잘못된 키 / 숫자 키 아님" 사유로 실패하면 (예: GitHub `feature-x` 입력, GitLab 에 없는 reference, Jira 미인증 후 재시도) 임의 spawn 진행하지 말고 다음 프로토콜로 사용자 결정 받기.

   1) `<issue_fetch_step>` 두 번째 줄의 search/list 명령 그대로 실행해 후보 ≤ 5 건 수집 (번호·제목·label).
   2) orch 경유 사용자 질문 — **leader 가 AskUserQuestion 직접 호출 금지** (허브 위반).
      ```bash
      qid="q-$(date +%s)-$RANDOM"
      bash -c "$ORCH_BIN_DIR/send.sh orch <<ORCH_MSG
      [issue-clarify:$qid <issue_display>]
      [question:$qid]
      입력한 issue-id '<issue_display>' 를 트래커에서 찾지 못했음.
      검색 결과 (상위 N 건):
      - <num1> <title1> [<labels1>]
      - <num2> <title2> [<labels2>]
      - ...
      어느 것으로 진행할지, 다른 ID 로 재시작할지, 또는 취소(--no-issue 또는 issue-down) 인지 알려달라.
      ORCH_MSG"
      bash $ORCH_BIN_DIR/wait-reply.sh $qid
      ```
   3) orch 가 사용자에게 후보 보여주고 `[issue-decision:$qid]` + `[reply:$qid]` 라벨로 회신. 회신 본문이:
      - 후보 중 하나의 ID/번호 → 그 ID 로 primary fetch 재시도 후 진행 (이 leader 의 `<issue_display>` 변수 값은 그대로 두고 fetch 결과만 갱신).
      - "다른 ID" / "취소" → orch 한테 다음 단계 결정 escalate (필요 시 사용자에게 `/orch:issue-down <mp_id>` 후 정확한 ID 로 `/orch:issue-up` 재호출 요청).
   4) 다른 qid 의 `[issue-decision:...]` 가 보이면 그 라운드는 무시 — 본인 qid 박힌 `[reply:<qid>]` 답만 자기 결정으로.
...
5. **항상 현재 phase 의 워커만 spawn.** 다음 phase 워커는 현재 phase 완료 보고 (PR merged + 로컬 동기화) 후 spawn. 동시 다중 phase 진행 금지. 단 §3.5 task graph 가 적용된 Phase 1 (execution) 안에서는 `depends_on` 충족된 ready task 를 병렬 spawn 가능 — 이는 같은 phase 내 동시 spawn 이라 phase 간 동시 진행이 아니다.
...
4. `[phase-plan]` 송신 본문: phases.md + Task Graph Summary (approved_task_graph.tasks 의 id / project / role / depends_on 요약 + ready wave).
...
3. `[phase-plan]` 송신 본문: phases.md + "PM 산출 예정 design artifact 6개 — Round 2 에서 task graph summary 와 함께 재컨펌" 한 줄 명시.
...
3. `[phase-plan]` 재송신 — 본문에 phases.md 본문 + "Task Graph Summary" (approved_task_graph.tasks 의 id / project / role / depends_on 요약 + ready wave). Round 2 임을 본문에 표시 ("Round 2 — task graph 확정").
...
- **단순 이슈** — leader 가 task-graph.json 의 design + execution 두 절을 1 라운드로 작성 후 `[phase-plan]` 송신 (본문: phases.md + Task Graph Summary). `[plan-confirm] GO` 후 §3.5.4 ready spawn.
