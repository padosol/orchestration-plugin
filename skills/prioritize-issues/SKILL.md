...
description: 워크스페이스에 등록된 이슈 트래커(Linear / GitHub) 의 미완료 이슈를 분석해 루브릭 점수 기반 Top N (기본 3) 을 추천한다. /orch:prioritize 호출 시 사용. 메인 컨텍스트 부담을 줄이기 위해 list/get 호출은 서브에이전트(Agent general-purpose) 로 위임하고 메인은 점수표 + 추천만 받는다. issue_tracker=none 워크스페이스에서는 NA 로 종료.
---
...
핵심 원칙: **메인 컨텍스트는 결과 (점수표 + Top N + 추천 next) 만 받는다.** 이슈 description fetch 같은 무거운 read 는 반드시 `Agent(general-purpose)` 서브에이전트에 위임. 메인이 직접 list_issues / get_issue / gh issue view 를 호출하면 description 이 메인 컨텍스트에 누적되어 후속 작업 효율이 떨어진다.
...
- **메인에서 직접 list_issues / gh issue list 호출 금지** — 컨텍스트 효율이 이 스킬의 핵심 이유. 위임 안 하면 스킬 만든 의미 없음.
