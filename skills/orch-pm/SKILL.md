...
ORCH_MSG"
bash $ORCH_BIN_DIR/messages/wait-reply.sh $qid    # 차단. leader 가 사용자에게 직접 확인 후 [reply:<qid>] 로 회신할 때까지 대기.
```
...
`references/orch-protocols.md` 4절의 HOLD 체크포인트를 그대로 따른다. PM 의 두 마디:
...
`references/orch-protocols.md` 5절의 PR 4단계를 그대로 따른다. PM 산출물 (스펙 / 다이어그램 / 스키마) 도 PR 로 통합 — 메인 브랜치에 들어가야 후속 developer 가 의존할 수 있다.

1. **CI** — 산출물 commit + push + `<pr_create_cmd>`. `<pr_checks_watch_cmd>`. 명령은 first_msg 에 git_host (github/gitlab) 별로 주입.
2. **리뷰** — 통과 후 leader 에 'PR #N ready for review + URL' 답신. reviewer 가 docs/spec 도 검토 (정합성·완전성 기준).
   - needs-changes → 수정 후 're-review please'
   - LGTM → 즉시 3 진입
3. **머지 대기** — `bash $ORCH_BIN_DIR/issues/wait-merge.sh <pr-num>` 30s 폴링.
4. **자기 종료** — `bash $ORCH_BIN_DIR/issues/worker-shutdown.sh` 한 번.
