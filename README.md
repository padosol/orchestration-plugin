...
3. tmux 세션을 새로 시작 (또는 `source ~/.bashrc`) — orch / leader / worker pane 모두 환경변수 상속.
4. 동작 확인 — 임의로 `notify-slack.sh` 직접 호출:
   ```bash
   "$CLAUDE_PLUGIN_ROOT/scripts/notify-slack.sh" mp_done MP-test "동작 확인" "/tmp/test"
   ```
5. (선택) 임시 비활성화: `export ORCH_NOTIFY_ENABLED=0`. URL 만 빼면 자동 noop.

**주의**:
- 실패는 조용히 흡수 (호출자 본 흐름 절대 안 막음). curl 5초 timeout.
- webhook URL 은 **민감 정보** — git 커밋 / 공유 채널 노출 금지.
