...
→ MP-13 leader pane 이 뜨고 Linear 이슈를 읽어 plan 을 orch 인박스로 보고 → 사용자가 confirm → leader 가 `/orch:leader-spawn server fix` 등으로 워커 spawn → 워커 PR → reviewer → 머지 → `/orch:issue-down MP-13` 으로 정리.
