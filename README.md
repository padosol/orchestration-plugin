...
이 plugin은 로컬 marketplace `~/.claude-marketplaces/local/`에 들어 있다.
...
해당 프로젝트의 Claude 세션에서:

```
/plugin install orch@local
```

설치 시 scope를 **project**로 선택하면 `<project>/.claude/settings.json`에 `enabledPlugins`로 등록된다 (그 프로젝트에서만 활성화).

설치할 프로젝트:
- `/home/padosol/lol/` (orch 윈도우용)
- `/home/padosol/lol/lol-server/` (server 워커)
- `/home/padosol/lol/lol-ui/` (ui 워커)
- `/home/padosol/lol/lol-repository/` (repo 워커)
...
```bash
~/.claude-marketplaces/local/plugins/orch/scripts/up.sh
```
...
```bash
~/.claude-marketplaces/local/plugins/orch/scripts/down.sh
```

## 메일박스 위치

`/home/padosol/lol/.orch/`
