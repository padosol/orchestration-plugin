...
- `bash` / `python3` 시스템 기본.
- 시나리오 72 (task-graph-jsonschema-validate) 는 `python3 -m pip install 'jsonschema>=4.10'` 필요 (Draft 2020-12). import 실패 시 hard FAIL — schema 는 design-first task graph 의 핵심 계약이므로 skip 정책 안 씀.
...
| `13-poll-inbox-happy.sh` | 파일 inbox 에 이미 도착한 최신 메시지 즉시 출력 |
| `14-send-leader-worker-file-delivery.sh` | leader → worker 송신이 tmux 알림 없이 파일 inbox 에 queue 되는지 |
...
| `70-task-graph-schema-fields.sh` | task-graph / task-template schema 핵심 필드 / role / status enum grep |
| `71-developer-pr-v1-step-order.sh` | developer_pr_v1.json step 순서 / blocking / owner invariant |
| `72-task-graph-jsonschema-validate.sh` | schema self-check + 모든 task-template/*.json 을 task-template.schema.json 으로 validate |
