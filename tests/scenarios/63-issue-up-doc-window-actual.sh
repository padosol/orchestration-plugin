#!/usr/bin/env bash
# Regression guard: commands/issue-up.md 가 실제 scripts/issues/issue-up.sh 동작 (새 tmux 윈도우 생성)
...
if ! grep -q 'orch_new_window "\$mp_id"' "$src"; then
    echo "FAIL: scripts/issues/issue-up.sh 가 orch_new_window 를 호출하지 않음 — 본 테스트의 전제 변경됨" >&2
    exit 1
fi
...
# 4. gitlab 트래커 분기도 문서에 반영하고, 제거된 jira 안내는 없어야 한다.
for tracker in gitlab; do
    if ! grep -q "$tracker" "$doc"; then
        echo "FAIL: commands/issue-up.md 에 트래커 '${tracker}' 분기 안내 누락" >&2
        exit 1
    fi
done
if grep -qi 'jira' "$doc"; then
    echo "FAIL: commands/issue-up.md 에 제거된 jira 트래커 안내 잔존" >&2
    exit 1
fi
