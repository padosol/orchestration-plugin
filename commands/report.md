...
2. JSON 은 다음 8개 섹션 콘텐츠를 담음 (스키마는 아래 "JSON 스키마" 참조):
   ...
   - **토큰·시간 분석** — 모델별 토큰 합계 + 도구 분포 + **스킬·slash 명령·subagent 분포** + 큰 tool_result top-5 + 관찰 (Read 반복 / 도구 쏠림 의심). raw 데이터의 "스킬 호출 top-10" / "Slash 명령 호출 top-10" / "Subagent 호출 top-10" 섹션을 각각 `skill_distribution` / `command_distribution` / `subagent_distribution` 으로 매핑 — bash/Read 같은 raw 도구 카운트보다 의미 있는 신호.

     **정확한 숫자가 필요하면** `analyze-jsonls.py --json` 출력의 `skill_counts` / `command_counts` / `subagent_counts` 를 사용. 다만 이들은 **dict 형식** (`{"orch:setup": 3, ...}`) 이고 renderer 스키마는 **array of objects** 라 변환 필수. 예시 (jq 한 줄):
     ```bash
     # skill_counts → skill_distribution
     jq '.skill_counts | to_entries | map({skill: .key, count: .value})'

     # command_counts → command_distribution
     jq '.command_counts | to_entries | map({command: .key, count: .value})'

     # subagent_counts → subagent_distribution
     jq '.subagent_counts | to_entries | map({subagent: .key, count: .value})'
     ```
     카운트 0 인 카테고리는 dict 자체가 빈 객체 — 변환 결과도 빈 array 라 renderer 가 표 자동 생략.
