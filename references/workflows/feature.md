...
2. **인터페이스 설계** — 데이터 모델 / API 시그니처 / 상태 변화 / 호환성 영향. 큰 기능이면 PM 산출물 (`docs/spec/<id>/...`) 이 필요하므로 phase plan 에 **Phase 0: 분석/설계** (또는 첫 phase) 로 명시 → `[plan-confirm] GO` 받은 뒤 그 phase 시작 시점에 PM 워커 spawn. **GO 전 PM 포함 어떤 워커도 spawn 금지.**
