...
- **leader ↔ worker 전달은 파일 inbox + 폴링이 canonical (포인터 모델)** — `send.sh` 는 메시지를 `.orch/.../inbox/<wid>/` 디렉터리에 메시지당 포인터 `<sortable>-<id>.json` + 본문 `payloads/<id>.md` 두 파일로 적재하고, 수신자는 `/orch:poll-inbox` 또는 `/orch:check-inbox` 로 도착 순(FIFO) 읽는다. tmux `send-keys` 는 spawn 시 claude 기동 1회에만 쓰이며 메시지 전달 경로가 아니다. spawn 직후 부트스트랩도 push 가 아니라 SessionStart hook 이 inbox 의 첫 메시지(spawn-context)를 셸에서 직접 드레인해 plain stdout 으로 모델 컨텍스트에 주입하고 단건 archive 하는 방식.
...
새 leader / worker 세션의 첫 지시처럼 아직 메시지가 없을 수 있는 경우는 `/orch:poll-inbox` 를 사용한다. 이미 도착한 메시지 목록을 훑거나 HOLD 체크포인트에서 확인할 때는 `/orch:check-inbox` 를 사용한다.

```bash
bash $ORCH_BIN_DIR/messages/poll-inbox.sh --timeout 3600 --interval 5
```

- 메시지가 이미 있으면 즉시 최신 메시지 본문을 출력.
- 메시지가 없으면 파일 inbox 를 interval 마다 확인. tmux 입력 주입에 의존하지 않는다.
- 출력된 `msg_id` 는 처리 후 `inbox-archive.sh <msg_id>` 로 archive.
- `send.sh` 성공 출력의 msg_id 와 수신자의 archive msg_id 가 같은 추적 키다.

---

## 3. wait-reply.sh — 차단 질문 패턴
...
worker spawn-context (inbox 첫 메시지) 가 git_host (github/gitlab) 별로 `<pr_create_cmd>` / `<pr_checks_watch_cmd>` / `<pr_run_log_failed_cmd>` 를 주입한다. SKILL 본문은 그 변수만 참조 — gh / glab 분기 직접 안 함.
