...
- **leader ↔ worker 전달은 파일 inbox + 폴링이 canonical** — `send.sh` 는 메시지를 `.orch/.../inbox/*.md` 에 append 하고, 수신자는 `/orch:poll-inbox` 또는 `/orch:check-inbox` 로 읽는다. tmux `send-keys` 알림은 leader ↔ worker 기본 경로가 아니다.
